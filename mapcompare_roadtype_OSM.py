...
class MapCompare:
    ...
    def compare_maps(self, eval_filename, root_locations_filename):
        ...
        if len(rootlocations1) == 0 or len(rootlocations2) == 0:
...
def mapcompare_roadtype_OSM(data_path,dataset,solution,compute_mode,year,token):
    ...
    for roadclass in [49,41000,42000,43000,44000,45000,47000,51000,52000,53000,54000]:
        if not os.path.exists('../temp_output_roadtype/GraphSamplingToolkit-main/'+dataset+'_'+str(roadclass)+'_'+str(year-1)+'/groundtruth/'+dataset+'_'+str(year-1)+'_edges_osm.txt'):
            continue
        df = pd.DataFrame({})
            ...
            for b in [45]:#,30
                for t in [i*10]:#
                    for c in ['knn']:#,'wmm'     
                        ...
                        f_data2 = f_data + '/../../../temp_output_roadtype/GraphSamplingToolkit-main/'+ dataset +'_' + str(roadclass)+'_'+str(year2)
                        
                        f_ground_truth = "{folder}/groundtruth/{dataset}_{year}_{{type}}.txt".format(folder=f_data2, dataset=dataset, year = year2)
                        f_dataset = "{folder}/algorithm/{algorithm}/{dataset}_{year}_{{type}}.txt".format(folder=f_data, year = year, #{algorithm}_
                                                                                                                dataset=dataset,
                                                                                                                algorithm=solution)
                        f_results = "{folder}/evals2/{dataset}_{{file}}.txt".format(folder=f_data, dataset=dataset)
        ...
        df.to_csv('../output/'+dataset+'_'+str(year-1)+'_'+str(roadclass)+'_'+token+'_recall.csv', index=False)